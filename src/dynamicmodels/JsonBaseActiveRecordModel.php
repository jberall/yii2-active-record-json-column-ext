...
class JsonBaseActiveRecordModel extends \yii\db\ActiveRecord {
     ...
     * Mapped to the data_json in the beforeSave by Json::Encode($this->arrData)
     * 
    ...
    /**
     * if class_exists(dynamicModelName) loads the arrData<br>
     * @inheritdoc
    ...
    public function init() {
        ...
        if (class_exists($this->dynamicModelName)) {
            $this->arrData = new $this->dynamicModelName();
        } else {
    ...
    /**
     * @inheritdoc
     *  initilizes the $arrData with a new dynamicModelName Model based on the Json::decode(data_json)
     */
    public function afterFind() {
        parent::afterFind();
        $this->arrData = new $this->dynamicModelName((is_array($arr=Json::decode($this->data_json))) ? $arr : []);  
    }
    
    /**
     * Converts the arrData into a Json document using Json::encode.
     * Resets the attributes indexes to 0,1,2... of  type JsonBaseDynamicModel::ARROBJECTS_TYPE['ARRAY_OBJECTS']
     * @inheritdoc
     */
    public function beforeSave($insert) {
        array_walk($this->arrData->arrObjects,"self::resetArrays");
        $this->data_json = Json::encode($this->arrData);
    ...
    public function resetArrays($item, $key){   
         if ($item['type'] == JsonBaseDynamicModel::ARROBJECTS_TYPE['ARRAY_OBJECTS']) {
             $this->arrData->$key = array_values($this->arrData->$key);
         }
    }
    /**
     * @inheritdoc
     * Also loads the arrData.
     * 
     */
...
//        load the attributes if data exists
        if (isset($data[DynamicModelHelper::getModelPathName($this->dynamicModelName)])){
            $valid = $this->arrData->load($data,$formName) && $valid;
        }
        
        //loop thru array and load objects.
        foreach($this->arrData->arrObjects as $key => $obj) {
...
//        $attributes = $this->tempData;
        ...
        if (is_object($this->arrData)) {
            $valid = $this->arrData->validate($attributeNames, $clearErrors) && $valid;
        }
        return $valid;
    }     
    ...
    public function setDynamicModelScenerio($value){
        $this->arrData->scenario = GeneralHelpers::getScenerio($this->arrData,$value);

        foreach($this->arrData->arrObjects as $key => $obj) {
            
            $modelName = DynamicModelHelper::getModelPathName($obj['modelPath']);

            $array = $this->arrData->$key;
            if ($obj['type'] == JsonBaseDynamicModel::ARROBJECTS_TYPE['ARRAY_OBJECTS']){
                ...
                $this->arrData->$key->scenario = $this->getScenerio($this->arrData->$key,$value);
//                die('here');
            } else {
